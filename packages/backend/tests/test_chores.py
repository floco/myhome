...
def test_preview_next_due_from_completion_anchors_on_last_completion_when_chore_id_given(client, home_id, tmp_path):
    # A chore on a 30-day-from-completion schedule, last actually completed
    # 2026-08-10 -- its real (list) next-due is 2026-09-09. Previewing with
    # its choreId, without changing any schedule settings, must reproduce
    # that same date rather than projecting forward from today.
    doc = ChoreDocument(
        chores=[
            Chore(
                id="c-anchor", name="Water plants", emoji="🌱", periodDays=30,
                frequencyType="interval", frequency=30, frequencyMetadata={"unit": "days"},
                scheduleFromDue=False, nextDueDate="2026-09-09T00:00:00Z",
            )
        ],
        assignments=[],
        completions=[
            CompletionRecord(
                id="comp-1", choreId="c-anchor", completedAt="2026-08-10T00:00:00Z",
                scheduledDue="2026-08-10T00:00:00Z", notes="",
            )
        ],
    )
    ...
    resp = client.post(f"/api/homes/{home_id}/chores/preview-next-due", json={
        "frequencyType": "interval", "frequency": 30, "frequencyMetadata": {"unit": "days"},
        "scheduleFromDue": False, "nextDueDate": "2026-09-09T00:00:00Z", "choreId": "c-anchor",
    })
    assert resp.status_code == 200
    assert resp.json()["nextDueDate"] == "2026-09-09T00:00:00Z"
